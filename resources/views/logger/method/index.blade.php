...
@section('content')
    ...
    <div class="row mb-4">
        <div class="col">
            <a class="btn btn-primary" href="{{ route('method.create') }}">
    ...
    <form onchange="this.submit()">
        <div class="row">
            <div class="col-1">
                <label class="mb-2 fw-bold" for="{{ $form->limit['name'] }}">На стр.</label>
                <select class="form-control form-select" name="{{ $form->limit['name'] }}">
                    @foreach ($form->limit['options'] as $k => $v)
                        <option value="{{ $k }}"@if (request()->get($form->limit['name']) == $k) selected @endif>{{ $v }}</option> 
                    @endforeach
                </select>
            </div>
